#!/usr/bin/env bash
#### Lists all AWS IPv6 prefixes  using the public ip-ranges.json file.
...
## --help?
  ...
  if [[ ! -z $REGION && ! -z $SERVICE ]]; then
    jq -c "{ ipv6_prefixes: [.ipv6_prefixes[] | select(.region == \"$REGION\" and .service == \"$SERVICE\") | { ipv6_prefix: .ipv6_prefix }] | unique }" ip-ranges.json

  ## Show IP prefixes filtered by region
  elif [[ ! -z $REGION ]]; then
    jq -c "{ ipv6_prefixes: [.ipv6_prefixes[] | select(.region == \"$REGION\") | { ipv6_prefix: .ipv6_prefix }] | unique }" ip-ranges.json

  ## Show IP prefixes filtered by service
  elif [[ ! -z $SERVICE ]]; then
    jq -c "{ ipv6_prefixes: [.ipv6_prefixes[] | select(.service == \"$SERVICE\") | { ipv6_prefix: .ipv6_prefix }] | unique }" ip-ranges.json
