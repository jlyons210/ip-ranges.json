#!/usr/bin/env bash
#### Lists AWS IPv6 prefixes assigned to a specified AWS service using the public ip-ranges.json file.
...
jq -c "{ ipv6_prefixes: [.ipv6_prefixes[] | select(.service == \"${1^^}\") | { ipv6_prefix: .ipv6_prefix }] | unique }" ip-ranges.json
